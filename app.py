...
try:
    ...
    OCR_AVAILABLE = False
    st.error("pytesseract not available")
...
try:
    ...
    st.error("pdf2image not available")
...
def main():
    ...
    if not all([OCR_AVAILABLE, PDF2IMAGE_AVAILABLE, PYPDF2_AVAILABLE]):
        st.warning("⚠️ Some dependencies not available. Using demonstration mode.")
    ...
    if uploaded_files and st.button("🚀 Process PDFs", type="primary"):
        ...
        list_of_health_labels = [
            "Employee Name", "IPPIS Number", "Total Gross Earnings", "Total Net Earnings"
        ]
